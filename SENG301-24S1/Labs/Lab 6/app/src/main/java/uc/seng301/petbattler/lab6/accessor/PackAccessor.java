package uc.seng301.petbattler.lab6.accessor;
...
public class PackAccessor {
    public static final Logger LOGGER = LogManager.getLogger(PackAccessor.class);
     ...
     * @param pets   The pets to be in the pack, must not be null and must have at
     *               least 1 card
    ...
    public Pack createPack(String name, Player player, List<Pet> pets) {
        ...
        if (!player.getPacks().stream().filter(pack -> pack.getName().equals(name)).toList().isEmpty()) {
     ...
     * @return Pack with id given if it exists in database, no player object
     *         attached; null if not found
     */
     ...
     * @throws UnsupportedOperationException if the pack already exists and should
     *                                       be updated instead
     */
    public Long persistPack(Pack pack) throws UnsupportedOperationException {
        ...
        if (null != pack.getPackId() && null != getPackById(pack.getPackId())) {
            throw new UnsupportedOperationException("Pack already exists, do you want to updatePack() instead?");
        }
        try (Session session = sessionFactory.openSession()) {
     ...
     * @return The matching pack
     */
    public Pack getPackByPlayerNameAndPackName(String playerName, String packName) {
        LOGGER.info("Getting pack from persistence by player name {} and pack name {}", playerName, packName);
        if (playerName == null || playerName.isEmpty()) {
            throw new IllegalArgumentException("cannot retrieve pack with null or empty player name");
        }
        ...
        try (Session session = sessionFactory.openSession()) {
            Player player = session.createQuery("FROM Player WHERE name='" + playerName + "'", Player.class)
                    .uniqueResult();
            Query query = session.createQuery("FROM Pack WHERE name=:packName AND player=:player", Pack.class);
    ...
    public long updatePack(Pack pack) throws UnsupportedOperationException {
        ...
        if (pack.getPackId() == -1L) {
            throw new UnsupportedOperationException("Pack has not been persisted, use persistPack() instead");
        }
        try (Session session = sessionFactory.openSession()) {
            Transaction transaction = session.beginTransaction();
            session.merge(pack);
