...
void *collatzSweep(void *arg) {
	...
	uint64_t *totalSteps = malloc(sizeof(uint64_t));

    if (totalSteps == NULL) {
        perror("Failed to allocate memory for total steps");
...
int main(void) {
    ...
    for (size_t i = 0; i < NUM_TESTS; i++) {
        ...
        printf("Sweep to %u: %lu total steps\n", testValues[i], *results[i]);
        free(results[i]); // Free the allocated memory for each result
    }
